...
@author: Henry Torlao Jr. Gikan kang sir Neil Arellano Mutia
"""
#Activity 2
...
# Load dataset-----------------------------------------------------------------
dataset = pd.read_csv('banana_disease_data_numerical.csv')
...
columns = list(dataset_cleaned.columns[:-1]) + ['Eligible_for_bonus']
merged_new_dataset = pd.DataFrame(merged_data, columns=columns)

# Export dataset cleaned to CSV---------------------------------------------------- 15.
merged_new_dataset.to_csv('dataset_cleaned_standardized.csv', index=False)
print("Cleaned dataset saved to 'dataset_cleaned_standardized.csv'.")